...
You run from the control directory `goal/` (the directory containing this file). It contains:
...
3. Prove parity per repo: trigger the GitHub-hosted fallback lane (`gh workflow run ...` on the dispatch escape hatch) AND the Velnor default lane — both must go green from the same workflow definition. Record the dispatch run links in the per-repo report.
...
5. Repeat until all 36 repos satisfy all three objectives.
